...
async function handleClientFileSelected(e) {
    ...
    try {
        ...
        if (err.data && err.data.missingFields) {
          let message = '❌ Colonnes obligatoires manquantes\n\n';
          
          err.data.missingFields.forEach(field => {
            const variants = expectedColumns[field] || [];
            message += `📋 ${field.toUpperCase()}\n`;
            message += `   Noms acceptés: ${variants.join(', ')}\n\n`;
          });
          
          message += '📄 En-têtes détectés:\n';
          message += err.data.detectedHeaders.join(', ') + '\n\n';
          
          if (err.data.unmappedColumns && err.data.unmappedColumns.length > 0) {
            message += '⚠️ Colonnes non reconnues:\n';
            message += err.data.unmappedColumns.join(', ');
          }
          alert(message);
